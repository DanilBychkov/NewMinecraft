...
namespace Minecraft.Models
{
    public class Zombi:ICreature
    {
            ...
            while (queue.Count != 0)
            {
                ...
                for (var dy = -1; dy <= 1; dy++)
                    for (var dx = -1; dx <= 1; dx++)
                    {
                        ...
                        {
                            if (!IsWay(point,map, decorationObject, new Point() { X = dx * 5, Y = dy * 5 },40)) continue;
        ...
        private static bool IsWay(Point pointPlayer, int[,] map, int[] decorationObject, Point delta, int size)
        {
