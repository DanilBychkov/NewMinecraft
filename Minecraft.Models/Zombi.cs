...
namespace Minecraft.Models
{
    public class Zombi:ICreature
    {
        ...
        public void GetNextPointToPlayer(int[,] map, ICreature player, int[] decorationObject, ScreenPoint ChangingScreen)//
        {
        ...
        private static Dictionary<Point, SinglyLinkedList<Point>> FindWays(int[,] map, Point start, ScreenPoint ChangingScreen, int[] decorationObject, Point end)//
        {
            ...
            while (queue.Count != 0)
            {
                ...
                for (var dy = -1; dy <= 1; dy++)
                    ...
                    {
                        if (dx == 0 && dy == 0) continue;//убрать потом
                        ...
                        if (!ways.ContainsKey(nextPoint))
                        {
                            ...
                            visited.Add(nextPoint);//добавить после проверки
        ...
        {
            if (Math.Abs(pointPlayer.X - pointMonster.X) < 40 && Math.Abs(pointMonster.Y - pointPlayer.Y) < 40) return true;
            return false;
        }

        public static bool IsWay(Point pointPlayer, ScreenPoint ChangingScreen, int[,] map, int[] decorationObject, Point delta, int size)
        {
        ...
        private static bool IsDecorationObject(Point playerPoint, ScreenPoint ChangingScreen, int[,] map, int[] decorationObject, Point delta)
        {
        ...
        public static  bool IsIntoBall(Point pointPlayer, Point objectPoint, int radious)
        {
