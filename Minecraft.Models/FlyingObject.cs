...
namespace Minecraft.Models
{
    public class FlyingObject
    {
        ...
        {
            if (positionPlayer.X!=goalPosition.X&&positionPlayer.Y!=goalPosition.Y)
        ...
        private List<Point> ChangePointsSkeleton(Point startPoint, Point finishPoint,int size)//не согласен с тем что передают в качестве size
        {
            var newPosition = new Point() { X = startPoint.X +size/2, Y = startPoint.Y+size/2 };//обратить внимание
            var resoult = new List<Point>();
            if (newPosition.X - finishPoint.X == 0 && newPosition.Y - finishPoint.Y == 0)
                return resoult;
            else if (newPosition.X - finishPoint.X == 0)
                return AddPoint(1, new Point() { X = 0, Y = -(Math.Abs(newPosition.Y - finishPoint.Y) / (newPosition.Y - finishPoint.Y)) },
                    Math.Abs(newPosition.Y - finishPoint.Y) + size, newPosition);
            else if (newPosition.Y - finishPoint.Y == 0)
                return AddPoint(1, new Point() { X = -(Math.Abs(newPosition.X - finishPoint.X) / (newPosition.X - finishPoint.X)), Y = 0 },
                    Math.Abs(newPosition.X - finishPoint.X) + size, newPosition);
            else
                return AddPoint( Math.Abs((newPosition.X - finishPoint.X) * 1.0 / (newPosition.Y - finishPoint.Y)),
                    new Point()
                    {
                        X = -(Math.Abs(newPosition.X - finishPoint.X) / (newPosition.X - finishPoint.X)),
                        Y = -(Math.Abs(newPosition.Y - finishPoint.Y) / (newPosition.Y - finishPoint.Y))
                    },
                    Math.Abs(newPosition.X - finishPoint.X) + size, newPosition);
        }
